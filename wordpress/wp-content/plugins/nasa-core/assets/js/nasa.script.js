...
/**
 * Click Select options Load quick add variations product in grid
 */
$('body').on('nasa_after_click_quick_add', function(e, _btn) {
    ...
    if (
        ...
        var _urlAjax = nasa_ajax_params.wc_ajax_url.toString().replace('%%endpoint%%', 'nasa_call_variations_form_loop');
            ...
            $.ajax({
                ...
                success: function(res) {
                    if (typeof res !== 'undefined') {
                        $(_btn).parents('.product-img-wrap').prepend(res);
                        $('.nasa-variations-quick-add.nasa-product-'+ _pid).remove();
                        $(_btn).removeClass('nasa-quick-add');
                    }

                    var _form_var = $(_btn).parents('.product-item').find('.variations_form');
                    var _first_img = $(_btn).parents('.product-item').find('.main-img img:first-child');

                    if(_form_var) {
                        $('body').trigger('init_quickview_variations_form', [_form_var]);

                        $(_first_img).attr('data-o_href',$(_first_img).attr('src'));
                        $(_btn).parents('.product-item').addClass('ns-var-loaded ns-var-active');
                        $(_form_var).addClass('ns-active').append('<a href="javascript:void(0);" title="Close" class="pe-7s-angle-down ns-close-quickadd" rel="nofollow"></a>');
                        $(_form_var).find('>div').removeClass().addClass('nasa-color-round nasa-label-round nasa-image-extends');
