...
class Nasa_WC_Attr_UX extends Nasa_Abstract_WC_Attr_UX {
    ...
    public function product_content_variations_ux_loop($return = false, $callAjax = false) {
        ...
        if (!$this->_content) {
            ...
            if ((isset($nasa_opt['loop_layout_buttons']) && in_array($nasa_opt['loop_layout_buttons'], $this->events_click))) {
                if (!empty($this->_outputs[self::_NASA_COLOR]) || !empty($this->_outputs[self::_NASA_IMAGE])) {
                    $class_wrap_color_image = 'nasa-product-content-color-image-wrap ns-modern-8 nasa-flex flex-column align-start';
            ...
            if ((isset($nasa_opt['loop_layout_buttons']) && in_array($nasa_opt['loop_layout_buttons'], $this->events_click))) {
                $arr = array("nasa-attr-ux-item" => "nasa-attr-ux-item-modern-8","nasa-active" => "");
                
                $this->_content = strtr($this->_content,$arr);
            }
            ...
            foreach ($this->_outputs[self::_NASA_COLOR] as $attr_name => $objs) {
                ...
                foreach ($terms as $term) {
                    if (in_array($term->slug, $array_keys)) {
                        ...
                        $this->_content .= sprintf(
                            '<a href="javascript:void(0);" class="' . $class . ' nasa-attr-ux-' . self::_NASA_COLOR . ' nasa-attr-ux-%s %s" data-value="%s" data-pa="%s" data-act="%s" rel="nofollow">%s</a>',
                            esc_attr($term->slug),
                            $active_curr ? 'nasa-active' : '',
                            esc_attr($term->slug),
                            sanitize_title($attr_name),
    ...
    public function get_nasa_attr_ux_html($html, $args) {
        if ((!isset($_REQUEST['wc-ajax']) || $_REQUEST['wc-ajax'] !== 'nasa_quick_view') && (!isset($_REQUEST['wc-ajax']) || $_REQUEST['wc-ajax'] !== 'nasa_call_variations_form_loop') && !is_product()) {
            return $html;
        }
    ...
    public function nasa_attr_ux_html($html, $term, $attr, $args) {
        ...
        switch ($attr->attribute_type) {
            ...
            case self::_NASA_COLOR:
                $html = sprintf(
                    '<a href="javascript:void(0);" class="nasa-attr-ux nasa-attr-ux-color nasa-attr-ux-%s' . $selected . '" data-value="%s" rel="nofollow">%s<span class="nasa-attr-text">%s</span></a>',
                    esc_attr($term->slug),
                    esc_attr($term->slug),
                    $color_html,
                    $name
    ...
    protected function _key_cache($product_id) {
        ...
        $key = (isset($nasa_opt['loop_layout_buttons']) && in_array($nasa_opt['loop_layout_buttons'], $this->events_click)) ? $product_id . '_nsm_8' : $product_id;
